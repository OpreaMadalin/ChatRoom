...
public class PBKDF2Hasher implements HashAlgorithm {
    ...
    public String saltAndHash(String str) {
        ...
        byte[] dk = pbkdf2(str.toCharArray(), salt, 1 << cost);
    ...
    public boolean checkPassword(String hashPass, String candidate) {
        Matcher m = layout.matcher(hashPass);
        if (!m.matches())
            throw new IllegalArgumentException("Invalid token format");
        int iterations = iterations(Integer.parseInt(m.group(1)));
        byte[] hash = Base64.getUrlDecoder().decode(m.group(2));
        byte[] salt = copyOfRange(hash, 0, SIZE / 8);
        byte[] check = pbkdf2(candidate.toCharArray(), salt, iterations);
        int zero = 0;
